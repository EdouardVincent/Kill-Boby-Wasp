...
swatter_image = pygame.image.load('C:\Jeu personnages\OIP (51).png')
...
while game_started :
    ...
    while game :
        swatter.x = pygame.mouse.get_pos()[0] 
        swatter.y = pygame.mouse.get_pos()[1]
        ...
        swatter_rect = swatter_image.get_rect(topleft = (swatter.x, swatter.y))

        bee_rect = bee_image.get_rect(topleft = (player.x,player.y))
        ...
        for event in pygame.event.get() :
            ...
            if event.type == MOUSEBUTTONDOWN :
                if bee_rect.collidepoint (swatter.x, swatter.y) :
                    lose = 2
        ...
        if lose == 2 :
            texte2 = font2.render("DEAD !",1,(255,0,0))
            window.blit(texte2,(700,330))
            pygame.display.flip()
            time.sleep(5)
            lose = 0
            game = 0
